...
def obtener_informacion_sitio(sitio):
    sitios = {
        'a': {'nombre': 'Servers', 'serial': 'Q2LW-3D2D-LP5T', 'interface_id_Servers': '754915887537981876'},   # el serial sería el del core y el id de cada svi
    ...
    if sitio in sitios:
        ...
        if interface_id:
            ...
            if response.status_code == 200:
                ...
                # Guardar la respuesta en un archivo CSV
                output_file = f"C:/RootScripts/Appelt/MW/DHCPConfigFor{nombre_sitio}.csv"                #<<<<< Se cambia variable en este caso el path
                with open(output_file, "w", newline="") as csvfile:
                    # Crear el escritor CSV
                    writer = csv.writer(csvfile)
