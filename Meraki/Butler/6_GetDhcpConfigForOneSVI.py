...
def obtener_informacion_sitio(sitio):
    sitios = {
        'a': {'nombre': 'Servers', 'serial': 'Q2LW-8A2B-5GJM', 'interface_id_Servers': '754915887537980885'},   # el serial sería el del core y el id de cada svi
    ...
    if sitio in sitios:
        ...
        if interface_id:
            ...
            if response.status_code == 200:
                ...
                # Guardar la respuesta en un archivo CSV
                output_file = f"C:/RootScripts/Butler/MW/DHCPConfigFor{nombre_sitio}.csv"                #<<<<< Se cambia variable en este caso el path
                with open(output_file, "w", newline="") as csvfile:
                    # Crear el escritor CSV
                    writer = csv.writer(csvfile)
