import requests
from pprint import pprint

url = "https://pokeapi.co/api/v2/pokemon/"  # un sitio que puedas hacer llamadas 

resp = requests.get(url)

respjson = resp.json()


# si quiero solo las llaves del diccionario:
#pprint(respjson)
for i in respjson:
    print(i)

print("***************************************************************")
# Si quiero algún valor de uno de las llaves por ejemplo "next":

print(respjson['next'])


print("***************************************************************")

# Si quiero todos los valores de las llaves:

#for i in respjson.values():
#    pprint(i)
for i in respjson:  # primero obtengo las llaves con un for 
    pprint(respjson[i]) # despues que tengo la llave en la variable de for que normalmente se maneja con "i" ponemos en cada interación respjson[i] osea printea la igualación de respjson[i]

print("***************************************************************")

#Quiero la url de charmeleon:


for i in respjson['results']:
    if  i['name'] == 'charmeleon':
        print(i['url'])

print("***************************************************************")

#Quiero todos los nombres de los pokemons:

for i in respjson['results']:
    print(i['name'])